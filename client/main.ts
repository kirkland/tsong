...
const net = connect(
  (msg) => {
    if (msg.type === 'you') {
      ...
      else if (msg.game === 'hockey') worldMod?.feedHockeyLobby(msg);
      ...
      else if (msg.game === 'hockey') worldMod?.feedHockeyRelay(msg.data);
      ...
      worldMod?.feedBoom(msg.x, msg.y, msg.r, msg.pid, msg.fx);
    ...
    } else if (msg.type === 'worldBoulder') {
      worldMod?.feedBoulder(msg.x, msg.y);
...
worldBtn.addEventListener('click', async () => {
  try {
    ...
    worldMod.startWorld({
      ...
      // The Boulder — stream its position while we're the one shoving it.
      boulderMove: (x, y) => net.send({ type: 'boulderMove', x, y }),
      ...
      hockeyJoin: () => net.send({ type: 'bgJoin', game: 'hockey' }),
      hockeyLeave: () => net.send({ type: 'bgLeave', game: 'hockey' }),
      hockeyStart: () => net.send({ type: 'bgStart', game: 'hockey' }),
      hockeyResult: (winner) => net.send({ type: 'bgResult', game: 'hockey', winner }),
      hockeyRelay: (data) => net.send({ type: 'bgRelay', game: 'hockey', data }),
      muted: () => muted,
      ...
      boom: (x, y, r, fx) => net.send({ type: 'worldBoom', x, y, r, fx }),
      rocket: (x, y, a, w, len) => net.send({ type: 'worldRocket', x, y, a, w, len }),

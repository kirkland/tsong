...
export const CAMPAIGN_STAGES: CampaignStage[] = [
  ...
  },
  {
    ...
    defeat: [
      ...
      { text: 'Sixty shots. That’s all I could skim from his books. Use them sparingly — they won’t last three rounds if you waste them.', sfx: '/blaster.mp3' },
    ],
  },
  {
    id: 'davis',
...
async function runCampaign(ctx: RunCtx, close: () => void) {
  ...
  for (let i = 0; i < CAMPAIGN_STAGES.length; i++) {
    ...
    if (cancelled()) return;
    if (stage.id === 'avery') ammo = 60; // Avery hands over the blaster before Davis
  }
